<?php
/**
 * Class WPHealthTracker_Exercise_Ajax_Functions - wphealthtracker-exercise-ajax.php
 *
 * @author   Jake Evans
 * @category Admin
...
if ( ! class_exists( 'WPHealthTracker_Exercise_Ajax_Functions', false ) ) :
	/**
	 * WPHealthTracker_Exercise_Ajax_Functions class. Here we'll house all of the Ajax Callback functions we'll need to
	 * make the Exercise tab run as expected. The initial functions that send calls to the functions in this file are
	 * located in the wphealthtracker-exercise-js.js JavaScript file.
	 */
	class WPHealthTracker_Exercise_Ajax_Functions {
		 ...
		 */
		public function wphealthtracker_jre_selecteduser_exercise_enter_action_callback() {
			global $wpdb;
			check_ajax_referer( 'wphealthtracker_jre_selecteduser_exercise_enter_action_callback', 'security' );
			...
			// If no data for today was found, create the userdailydata object that 'class-exercise-forms-actual.php' will use just to be verbose, prevent PHP error messages, and to pass today's date and first and last name.
			if ( ! $todayflag ) {
				...
				$userdailydata->exercisestring = '';
				$userdailydata->exerciseimg    = '';
				$userdailydata->exercisefiles  = '';
			}

			// Require the file that contains the class that will output the forms needed for the Exercise tab.
			require_once WPHEALTHTRACKER_EXERCISE_DIR . 'class-wphealthtracker-exercise-forms-actual.php';

			// Instantiate the class and get the final output for the current day's data.
			$tabformclass_today = new WPHEALTHTRACKER_Exercise_Forms_Actual();
		 ...
		 */
		public function wphealthtracker_jre_selecteduser_exercise_view_action_callback() {
			global $wpdb;
			check_ajax_referer( 'wphealthtracker_jre_selecteduser_exercise_view_action_callback', 'security' );
			$wpuserid = filter_var( $_POST['wpuserid'], FILTER_SANITIZE_NUMBER_INT );
			$offset   = filter_var( $_POST['offset'], FILTER_SANITIZE_NUMBER_INT );
			...
			// Instantiate the class and get the final output for the current day's data.
			$tabformclass_previous = new WPHEALTHTRACKER_Exercise_Forms_Actual();
		...
		/**
		 * Callback function for the wphealthtracker_jre_selecteduser_exercise_filter_data() function found in wphealthtracker-exercise-js.js - For grabbing and outputting data when the 'Filter' button is clicked...
		 */
		public function wphealthtracker_jre_selecteduser_exercise_filter_data_action_callback() {
			global $wpdb;
			check_ajax_referer( 'wphealthtracker_jre_selecteduser_exercise_filter_data_action_callback', 'security' );
			...
			if ( false === $unset_flag && count( $useralldata ) > ( $filterval - 1 ) ) {
				array_pop( $useralldata );
			}
		...
		/**
		 * Callback function for the wphealthtracker_jre_save_exercise_data() function found in wphealthtracker-exercise-js.js - for saving user's data.
		 */
		public function wphealthtracker_jre_save_exercise_data_action_callback() {
			global $wpdb;
			check_ajax_referer( 'wphealthtracker_jre_save_exercise_data_action_callback', 'security' );

			$exercise_data = filter_var( $_POST['exerciseData'], FILTER_SANITIZE_STRING );
			$human_date    = filter_var( $_POST['humanDate'], FILTER_SANITIZE_STRING );
			$first_name    = filter_var( $_POST['firstName'], FILTER_SANITIZE_STRING );
			$last_name     = filter_var( $_POST['lastName'], FILTER_SANITIZE_STRING );
			$wpuserid      = filter_var( $_POST['wpUserId'], FILTER_SANITIZE_NUMBER_INT );

			$exercise_save_array = array(
				'exercisestring' => $exercise_data,
			...
			require_once WPHEALTHTRACKER_EXERCISE_DIR . 'class-wphealthtracker-save-exercise-data.php';
			$save_class      = new WPHEALTHTRACKER_Save_Exercise_Data( $exercise_save_array );
			$db_write_result = $save_class->wphealthtracker_jre_save_exercise_actual();
			...
			$return_array = array(
				...
				wp_json_encode( $save_class->exercise_save_array ),
			);

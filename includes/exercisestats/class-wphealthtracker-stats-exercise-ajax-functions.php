<?php
/**
 * Class WPHealthTracker_Stats_Exercise_Ajax_Functions - class-wphealthtracker-stats-exercise-ajax-functions.php
 *
 * @author   Jake Evans
 * @category Admin
...
if ( ! class_exists( 'WPHealthTracker_Stats_Exercise_Ajax_Functions', false ) ) :
	/**
	 * WPHealthTracker_Stats_Exercise_Ajax_Functions class. Here we'll house all of the Ajax Callback functions we'll need to
	 * make the Stats Exercise tab run as expected. The initial functions that send calls to the functions in this file are
	 * located in the wphealthtracker-stats-exercise-js.js JavaScript file.
	 */
	class WPHealthTracker_Stats_Exercise_Ajax_Functions {

		/**
		 * Callback function for grabbing the user's Dashboard Data & HTML.
		 */
		public function wphealthtracker_jre_grab_user_data_for_exercise_dashboard_action_callback() {

			global $wpdb;
			check_ajax_referer( 'wphealthtracker_jre_grab_user_data_for_exercise_dashboard_action_callback', 'security' );
			...
			if ( $transient_exists ) {
				...
				$query    = $wpdb->prepare( 'SELECT * FROM ' . $wpdb->prefix . 'wphealthtracker_user_daily_data_exercise WHERE wpuserid = %d', $wpuserid );
			...
			$dashboard = new WPHealthTracker_Dashboards( $userdata, $wpuserid, 'Exercise' );

			if ( count( $userdata ) === 0 ) {
				wp_die( esc_html( $dashboard->output_no_data() ) );
			} else {
				wp_die( esc_html( $dashboard->output_exercise_backend_dashboard() ) );
			}
		 ...
		 */
		public function wphealthtracker_jre_grab_user_data_for_exercise_d3_action_callback() {

			global $wpdb;
			check_ajax_referer( 'wphealthtracker_jre_grab_user_data_for_exercise_d3_action_callback', 'security' );
			$wpuserid = filter_var( $_POST['wpuserid'], FILTER_SANITIZE_NUMBER_INT );
			...
			// Require the D3 Exercise class.
			require_once WPHEALTHTRACKER_CLASSES_D3_EXERCISE_DIR . 'class-wphealthtracker-d3-exercise.php';
			$d3 = new WPHealthTracker_D3_Exercise( $userdata );

			// Getting all the data we'll need from the WPHealthTracker_D3_Exercise class - could have split up into 3 or 4 functions, but then we'd have at least 3 or 4 transient calls, which still impact performance even though they're transients, so this is better.
